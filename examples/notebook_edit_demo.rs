...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fs::write(&notebook_path, serde_json::to_string_pretty(&sample_notebook)?).await?;
    println!("Created sample notebook: {}\n", notebook_path.display());

    // Example 1: Replace cell content
    println!("1. Replacing content of cell-1...");
    let call = create_tool_call("call-1", "notebook_edit", json!({
    ...
    let call = create_tool_call("call-2", "notebook_edit", json!({
        "notebook_path": notebook_path.to_str().unwrap(),
        "cell_id": "cell-1",
        "cell_type": "code",
    ...
    let call = create_tool_call("call-3", "notebook_edit", json!({
        "notebook_path": notebook_path.to_str().unwrap(),
        "cell_type": "markdown",
    ...
    println!("\n4. Deleting cell-2...");
    let call = create_tool_call("call-4", "notebook_edit", json!({
        "notebook_path": notebook_path.to_str().unwrap(),
        "cell_id": "cell-2",
    ...
    let call = create_tool_call("call-5", "notebook_edit", json!({
        "notebook_path": notebook_path.to_str().unwrap(),
        "cell_id": "nonexistent",
