...
fn check_nodejs_available() -> SageResult<()> {
    let output = Command::new("node")
        .arg("--version")
        .output()
        .map_err(|e| SageError::Other(format!("Node.js not found. Please install Node.js 20+ to use the modern UI: {}", e)))?;
    
    if !output.status.success() {
        return Err(SageError::Other("Node.js is not working properly".to_string()));
    }
