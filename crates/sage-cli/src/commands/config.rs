...
use std::path::Path;
use sage_core::{
    config::{loader::load_config_from_file, model::{Config, TrajectoryConfig}},
...
pub async fn validate(config_file: &str) -> SageResult<()> {
    ...
    if !Path::new(config_file).exists() {
        return Err(SageError::config(format!("Configuration file not found: {config_file}")));
    }
        ...
        Ok(config) => {
            ...
            match config.validate() {
                Ok(()) => {
                    ...
                    console.info(&format!("Providers configured: {}", config.model_providers.len()));
...
fn print_config(console: &CLIConsole, config: &Config) {
    console.info(&format!("Default Provider: {}", config.default_provider.green()));
    console.info(&format!("Max Steps: {}", config.max_steps.to_string().yellow()));
    
    if let Some(working_dir) = &config.working_directory {
        console.info(&format!("Working Directory: {}", working_dir.display().to_string().cyan()));
    }
    ...
    for (name, params) in &config.model_providers {
        ...
        console.info(&format!("  API Key: {}", 
            if params.api_key.is_some() { "✓ Set".green() } else { "✗ Not set".red() }
        ));
    ...
    console.info(&format!("Enabled Tools: {}", config.tools.enabled_tools.len()));
    
    for tool in &config.tools.enabled_tools {
        console.info(&format!("  • {tool}"));
    }
    
    console.info(&format!("Max Execution Time: {}s", config.tools.max_execution_time));
    console.info(&format!("Parallel Execution: {}", 
        if config.tools.allow_parallel_execution { "✓ Enabled".green() } else { "✗ Disabled".red() }
    ));
...
fn create_sample_config() -> Config {
    ...
    use sage_core::config::model::{ModelParameters, ToolConfig, LoggingConfig};
    
    let mut model_providers = HashMap::new();
    
    // OpenAI configuration
    model_providers.insert("openai".to_string(), ModelParameters {
    ...
    model_providers.insert("anthropic".to_string(), ModelParameters {
