...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Default working directory: {:?}", config.default_working_directory);
    println!("Max execution time: {} seconds", config.max_execution_time_seconds);
    ...
    println!("Updated max execution time: {} seconds", updated_config.max_execution_time_seconds);
    ...
    record_error("json_edit", Duration::from_millis(50), "InvalidArguments".to_string());
    record_success("task_management", Duration::from_millis(25));
    record_error("bash", Duration::from_millis(300), "ExecutionFailed".to_string());
    ...
    let call = create_tool_call("demo-add", "add_tasks", json!({
        "tasks": [
            {
    ...
    println!("Overall success rate: {:.1}%", final_report.overall_success_rate * 100.0);
    
    if let Some(bash_metrics) = final_report.tool_metrics.iter().find(|m| m.tool_name == "bash") {
        println!("Bash tool average execution time: {:.1}ms", bash_metrics.average_execution_time_ms);
    }
