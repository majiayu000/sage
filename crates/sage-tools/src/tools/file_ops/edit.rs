...
- The edit will FAIL if `old_string` is not unique in the file. To fix this:
  1. Provide MORE CONTEXT: Include surrounding lines (e.g., function signature, class name, preceding comments) to make the match unique
  2. Or use `replace_all=true` if you want to change ALL occurrences of the same pattern
  Example: If a method signature like `def get_value(self):` appears in multiple classes, include the class definition or decorator above it.
