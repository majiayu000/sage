...
mod terraform_tool_tests {
    ...
    async fn test_terraform_tool_validate_without_init() {
        ...
        let generate_call = create_tool_call(
            "generate",
            "terraform",
            json!({
                "command": "generate",
        ...
        let validate_call = create_tool_call(
            "validate",
            "terraform",
            json!({
                "command": "validate",
                ...
                if let Some(props) = obj["properties"].as_object() {
                    for (param_name, param_schema) in props {
                        assert!(
                            ...
                            "Tool {} parameter {} is not an object",
                            schema.name,
                            param_name
                        );
                        ...
                        if let Some(param_obj) = param_schema.as_object() {
                            assert!(
                                param_obj.contains_key("description"),
                                "Tool {} parameter {} missing description",
                                schema.name,
                                param_name
                            );
                            assert!(
                                param_obj.contains_key("type"),
                                "Tool {} parameter {} missing type",
                                schema.name,
                                param_name
                            );
                        }
