...
impl Tool for AddTasksTool {
    ...
    fn parameters(&self) -> &[ToolParameter] {
        &[
            ToolParameter {
                ...
                description: "Array of tasks to create. Each task should have name and description.".to_string(),
                required: true,
    ...
    async fn execute(&self, tool_call: &ToolCall) -> SageResult<ToolResult> {
        let tasks_value = tool_call.arguments
            .get("tasks")
            .ok_or_else(|| SageError::tool("Missing required parameter: tasks".to_string()))?;

        let tasks_array = tasks_value.as_array()
            .ok_or_else(|| SageError::tool("Tasks parameter must be an array".to_string()))?;

        let mut created_tasks = Vec::new();

        for task_value in tasks_array {
            let task_obj = task_value.as_object()
                .ok_or_else(|| SageError::tool("Each task must be an object".to_string()))?;

            let name = task_obj.get("name")
                .and_then(|v| v.as_str())
                .ok_or_else(|| SageError::tool("Task name is required".to_string()))?;

            let description = task_obj.get("description")
                .and_then(|v| v.as_str())
                .ok_or_else(|| SageError::tool("Task description is required".to_string()))?;
...
impl Tool for UpdateTasksTool {
    ...
    fn parameters(&self) -> &[ToolParameter] {
        &[
            ToolParameter {
                name: "tasks".to_string(),
                description: "Array of tasks to update. Each task should have a task_id and the properties to update.".to_string(),
                required: true,
                parameter_type: "array".to_string(),
            }
        ]
    ...
    async fn execute(&self, tool_call: &ToolCall) -> SageResult<ToolResult> {
        ...
        for task_value in tasks_array {
            ...
            let task_id = task_obj.get("task_id")
                .and_then(|v| v.as_str())
                .ok_or_else(|| SageError::tool("Task task_id is required".to_string()))?;
