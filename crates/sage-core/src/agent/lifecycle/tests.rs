...
fn test_lifecycle_error_display() {
    let err = LifecycleError::init_failed("test".to_string());
    assert_eq!(err.to_string(), "Initialization failed: test");

    let err = LifecycleError::hook_failed(LifecyclePhase::TaskStart, "failed");
    assert!(err.to_string().contains("task_start"));

    let err = LifecycleError::invalid_transition(AgentState::Completed, AgentState::Thinking);
