...
pub struct ParallelToolExecutor {
    /// Registered tools
    pub(super) tools: DashMap<String, Arc<dyn Tool>>,
    /// Configuration
    pub(super) config: ParallelExecutorConfig,
    /// Global concurrency semaphore
    pub(super) global_semaphore: Arc<Semaphore>,
    /// Per-type semaphores for ExclusiveByType mode
    pub(super) type_semaphores: DashMap<String, Arc<Semaphore>>,
    /// Sequential execution lock (for Sequential mode)
    pub(super) sequential_lock: Arc<Mutex<()>>,
    /// Limited concurrency semaphores (tool name -> semaphore)
    pub(super) limited_semaphores: DashMap<String, Arc<Semaphore>>,
    /// Permission handler
    pub(super) permission_handler: RwLock<Option<SharedPermissionHandler>>,
    /// Permission cache
    pub(super) permission_cache: Arc<PermissionCache>,
    /// Tool context for permission checking
    pub(super) tool_context: RwLock<ToolContext>,
    /// Cancellation token
    pub(super) cancellation_token: CancellationToken,
    /// Execution statistics
    pub(super) stats: Arc<RwLock<ExecutorStats>>,
}
