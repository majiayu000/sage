...
impl BatchToolExecutor {
    ...
    async fn execute_parallel_batch(&self, calls: &[ToolCall]) -> Vec<ToolResult> {
        ...
        let (parallel_calls, sequential_calls): (Vec<_>, Vec<_>) = calls
            .iter()
            .partition(|call| {
                self.tools
                    .get(&call.name)
    ...
    async fn execute_concurrent_tools(&self, calls: &[&ToolCall]) -> Vec<ToolResult> {
        ...
        let chunks: Vec<_> = calls
            .chunks(self.max_parallel_tools)
            .collect();
    ...
    fn analyze_tool_calls(&self, calls: &[ToolCall]) -> BatchAnalysis {
        ...
        let parallel_capable = calls.iter().filter(|call| {
            self.tools
                .get(&call.name)
                .map(|tool| tool.supports_parallel_execution())
                .unwrap_or(false)
        ...
        analysis.should_parallelize = parallel_capable > 1 && calls.len() <= self.max_parallel_tools;
        analysis.parallel_tools = parallel_capable;
        analysis.sequential_tools = calls.len() - parallel_capable;
        analysis.has_expensive_tools = calls.iter().any(|call| {
            self.is_expensive_tool(&call.name)
        });
    ...
    fn reorder_results(&self, original_calls: &[ToolCall], mut results: Vec<ToolResult>) -> Vec<ToolResult> {
